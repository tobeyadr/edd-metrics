...
//			if ( false === $metrics ) {
			...
			$metrics = array(
				...
				'groundhogg'    => self::get_groundhogg_stats()
			);
...
//			}
		...
		public static function get_avg_yearly( $earnings = null, $previous_earnings = null, $num_days = null ) {
			...
			return array(
				'total'   => edd_currency_filter( edd_format_amount( $avgyearly ) ),
				'compare' => array(
					'classes'    => $classes,
					'percentage' => self::get_percentage( $avgyearly, $previous_avgyearly )
				)
			);
		...
		public static function get_sales() {
			...
			return array(
				...
				'compare'  => array(
					'classes'    => $classes,
					'percentage' => self::get_percentage( $sales, $previous_sales )
				)
			);
		...
		public static function get_renewals( $start = null, $end = null ) {
			...
			if ( ! class_exists( 'EDD_Software_Licensing' ) ) {
				return array(
					'count'    => '0',
					'earnings' => '0',
					'compare'  => array( 'classes' => 'edd-metrics-nochange', 'percentage' => '0' )
				);
			}
		...
		public static function compare_renewals( $current_renewals = null ) {
			...
			return array(
				'classes'    => $classes,
				'percentage' => self::get_percentage( $current_renewals, $previous_renewals[ 'count' ] )
			);
		}
		...
		public static function compare_discounts( $current_discounts_amount = null ) {
			...
			return array(
				'classes'    => $classes,
				'percentage' => self::get_percentage( $current_discounts_amount, $previous_discounts[ 'amount' ] )
			);
		}
		...
		public static function get_refunds() {
			...
			return array(
				'count'   => $refunds[ 'count' ],
				'losses'  => edd_currency_filter( edd_format_amount( $refunds[ 'earnings' ] ) ),
				'compare' => $compare,
				'integer' => $refunds[ 'earnings' ]
			);
		}
		...
		public static function compare_refunds( $current_refunds = null ) {
			...
			return array(
				'classes'    => $classes,
				'percentage' => self::get_percentage( $current_refunds, $previous_refunds[ 'count' ] )
			);
		}
		...
		public static function compare_subscriptions( $current_subscriptions = null ) {
			...
			return array(
				'previous_count' => $previous_subscriptions,
				'classes'        => $classes,
				'percentage'     => self::get_percentage( $current_subscriptions, $previous_subscriptions )
			);
		}
...
//		    $dates =self::get_compare_dates();
		...
		public static function get_lost_customers( $start = '', $end = '' ) {
			...
			global $wpdb;
			// get total
			$subscriptions = $wpdb->get_results( sprintf( "SELECT * FROM `wp_edd_subscriptions` WHERE STATUS = 'cancelled'" ) );
			...
			foreach ( $subscriptions as $subscription ) {
				...
				if ( $notes ) {
					foreach ( $notes as $key => $note ) {
						...
						if ( strpos( $note, __( "- Subscription #" . $sub->id . " cancelled", 'edd-recurring' ) ) !== false ) {

							if ( strtotime( substr( $note, 0, strpos( $note, '-' ) ) ) > $start && strtotime( substr( $note, 0, strpos( $note, '-' ) ) ) < $end ) {

								$other_status = $wpdb->get_results( sprintf( "SELECT count(customer_id) as total FROM `wp_edd_subscriptions` WHERE STATUS != 'cancelled' AND customer_id=%s", $subscription->customer_id ) );

								if ( absint( $other_status[ 0 ]->total ) > 0 ) {
									$lost ++;
								}
							}
							break;
